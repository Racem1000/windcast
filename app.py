...
def _engineer_features(df, turbulence_bins=None):
    ...
    if turbulence_bins is None:
        # Fallback: fit on whatever data is passed (only safe if this IS the training data)
...
def run_pipeline(df_input):
    ...
    df_raw = df.copy()   # will be overwritten after cleaning below
    ...
    if total_missing > 0:
        # For time-series: forward-fill then backward-fill; remaining → column median
    ...
    outlier_cols = [c for c in numeric_cols if c != "Power"]  # don't cap the target
    outlier_report = {}
    ...
    # data so the bin boundaries are never influenced by test-set values.
...
# ═════════════════════════════════════════════════════════════
    ...
    # Feature importances (not for NN)
...
# ═════════════════════════════════════════════════════════════
    ...
    def build_input_row():
        ...
        # Physically: if the wind is blowing at windspeed_100m now, the best
        # estimate for what it was 6/12/24h ago (absent other information) is
        # the same value. More defensible than using a historical median.
        for lag in [1, 2, 3, 6, 12, 24]:
        ...
        # P(v) = (v³ - v_cut_in³) / (v_rated³ - v_cut_in³)  for cut_in ≤ v < v_rated
        # P(v) = 1.0  for v_rated ≤ v < v_cut_out  (normalized to rated power)
        # P(v) = 0.0  otherwise
    ...
    if forecast_mode == "Best Model":
        ...
        # Ensemble: average across all loaded models (loaded once and cached)
        all_model_objects = load_ensemble_models()
        preds_ensemble = [
